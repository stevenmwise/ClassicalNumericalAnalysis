% This code defines a strongly convex and globally Lipschitz smooth
% quadratic objective and uses it to compare the performance of PSD and AGD
...
%
  E = 0.125*( L - mu )*( ( u - e1 )'*AA*( u - e1 ) ) + 0.5*mu*( norm(u)^2 );
end
...
% The minimum of the objective can be found by solving the Euler equations.
% Since the objective is quadratic these turn out to be a linear system of
% equations
uexact = AA*e1;
...
% We run the methods
[energyPSD, energyAGD, upsd, uagd] = RunMethods( E, Ep, mu, L, MaxIts, uinit );
...
title1 = ['OUT/ComparisonQuadratic', num2str(dim),'.pdf'];
print( hf, title1, "-dpdflatex");

title2 = ['OUT/ComparisonQuadratic', num2str(dim),'Gray.pdf'];
print( hf, title2, "-dpdflatex", "-mono");



% We now compare the rate of convergence of AGD with the theoretically
% optimal one
hff = figure(2);
clf;
kappa = sqrt( L/mu );
...
title3 = ['OUT/ComparisonAGDandOptimal', num2str(dim),'.pdf'];
print( hff, title3, "-dpdflatex");

title4 = ['OUT/ComparisonAGDandOptimal', num2str(dim),'Gray.pdf'];

print( hff, title4, "-dpdflatex", "-mono");
