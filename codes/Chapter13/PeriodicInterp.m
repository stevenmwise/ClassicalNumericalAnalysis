...
for n = [6,8,12,16]
    ...
    plot(xf,yf,'k-',xc,yc,'ko',xf,py,'k--','LineWidth',1.25)
    axis([-0.1,2.1,0.2,2.6])

    xlabel('$x$','Interpreter','latex');
    title(['Trigonometric Interpolation, $n=\mbox{}$',num2str(n)],'Interpreter','latex');
    legend('$f(x)$', '$f(x_j)$', 'interpolant',...
        'Interpreter','latex')
    printstr = strcat('TrigInterp',num2str(n),'.pdf');
