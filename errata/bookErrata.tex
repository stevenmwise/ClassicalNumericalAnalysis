...
\item Page 224, Problem~8.3a. The definition of the Cassini ovals must be
\[
  C_{i,j} = \left\{ z \in \Complex \ \middle| \ |z - a_{i,i}| |z - a_{j,j} | \leq R_i R_j \right\}.
\]
In other words, the inequality is not strict. In addition, notice that $C_{i,i} = D_i$, so the problem, as stated, is trivial. To make it meaningful, the display equation should read
\[
...
In other words, the term $\delta_\tau e^{k+1}$ should be either $\overline\delta_\tau e^{k+1}$ or $\delta_\tau e^k$.
  ...
  \item
Page 794, statement of Theorem 28.29. The first display equation should read
\[
  \| \mathcal{Z}[v] \|_{L^2(-\pi,\pi;\Complex)}^2 = 2 \pi h^2 \| v \|_{L^2_h}^2, \quad \forall v \in L^2_h(\polZ_h;\Complex).
\]
In other words, the factor $h$ must be squared.
	\end{enumerate}
